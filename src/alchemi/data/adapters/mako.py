...
def _to_sample(ds: xr.Dataset, spectrum: Any, position: tuple[int, int]) -> Sample:
    ...
    return Sample(
        ...
        band_meta={
            ...
            "srf_source": np.array(["mission"] * wavelengths.size, dtype=object),
            "srf_provenance": np.array(["none"] * wavelengths.size, dtype=object),
            "srf_approximate": np.array([True] * wavelengths.size, dtype=bool),
        },
