...
class MaskedAutoencoder(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        # For runs with masked spatial tokens, fill their outputs with a simple
        # baseline (mean over visible tokens) to stabilise the spatial loss.
        if masked_tokens.shape[1] > 0:
            decoded = decoded.clone()
            if visible_tokens.numel() == 0:
                fill_value = torch.zeros(batch, 1, bands, device=decoded.device, dtype=decoded.dtype)
            else:
                fill_value = visible_tokens.mean(dim=1, keepdim=True)
            decoded[:, ~spatial_mask] = fill_value.expand_as(decoded[:, ~spatial_mask])
        ...
        spectral_weight = 0.1 if self.mask_cfg.spatial_mask_ratio > 0 else 1.0
