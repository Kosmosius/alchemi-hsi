...
- **Lint clean** — `make lint` passes with no warnings (Ruff lint + Black formatting check + isort check-only). If Make is unavailable, run `ruff check src tests`, `black --check src tests`, and `isort --check-only src tests` manually.
- **Type safe** — `make typecheck` succeeds using **mypy** (configuration in `mypy.ini`).
