...
def _rectangular_srf() -> SensorSRF:
    ...
    return SensorSRF(wavelength_grid_nm=wl, srfs=srfs, band_centers_nm=centers, band_widths_nm=widths)
...
def test_resample_with_rectangular_srf_single_and_batch():
    ...
    spectrum = Spectrum(wavelengths=wavelengths, values=values, kind=QuantityKind.RADIANCE, units=RadianceUnits.W_M2_SR_NM)
    ...
    batched = Spectrum(wavelengths=wavelengths, values=batched_values, kind=QuantityKind.RADIANCE, units=RadianceUnits.W_M2_SR_NM)
...
def test_flat_in_flat_out():
    ...
    spectrum = Spectrum(wavelengths=wavelengths, values=flat_values, kind=QuantityKind.RADIANCE, units=RadianceUnits.W_M2_SR_NM)
