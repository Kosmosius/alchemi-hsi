...
def test_row_normalization_preserves_flat_spectrum():
    ...
    flat = Spectrum(
        wavelength_nm=wavelength_nm, values=np.ones_like(wavelength_nm), kind="radiance"
    )
...
def test_registered_srfs_have_unit_row_integrals(sensor_id: str):
    ...
    for idx, (nm, resp, is_bad) in enumerate(
        zip(srf.bands_nm, srf.bands_resp, bad_band_mask, strict=True)
    ):
        if is_bad:
            continue
