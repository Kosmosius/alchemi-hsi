...
@st.composite
def _paired_arrays(draw, *, min_size: int = 1, max_size: int = 6, wl_bounds: tuple[float, float], val_bounds: tuple[float, float]):
...
@given(_paired_arrays(wl_bounds=(8000.0, 14000.0), val_bounds=(240.0, 340.0)))
...
def test_radiance_bt_round_trip(data):
    ...
    wavelengths, temps = data
    radiance = planck.bt_K_to_radiance(temps, wavelengths)
    recovered = planck.bt_K_to_radiance(planck.radiance_to_bt_K(radiance, wavelengths), wavelengths)
